...
function Pixel({ color }) {
  return <div className="pixel" style={{ backgroundColor: color }}></div>;
}
function Icon({ iconData }) {
  ...
  async function iconClicked() {
    ...
    if (
      iconData.icons.length > 1 &&
      iconData.icons.length !== iconData.delays.length
    ) {
      throw new Error(
        "Icon data is invalid: frame data and frame delay arrays are of different length. Cannot send to device."
      );
    }

    let bufferIdx = 0;
    let rawIcon = new Uint8Array(
        ...
        return (
          <div key={row} className="pixel-row">
            {[...Array(8).keys()].map(col => {
              if (iconData.icons[frame] != null) {
                return renderPixel(
                  row * 4 + col,
                  iconData?.icons[frame][row][col]
                );
              } else {
                setFrame(0);
              }
